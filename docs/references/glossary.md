---
id: glossary
title: Glossary
---

<!--
Notes:
- Synonyms, Not To Be Confused With, and See Also should be omitted when they are empty.
- Avoid links in the definition. Add them to the sections in the note above.
- Definitions should be descriptive but brief. The Glossary is not a tutorial. 
-->

## Active cell
A cell in the current state of CKB. Active cells can be used as inputs to transactions.

---

## Aggron
The main public testnet for Nervos Network.

#### Synonyms
- [Testnet](#testnet)

#### Not To Be Confused With
- [Lina](#lina)
- [Mainnet](#mainnet)

---

## Animagus

---

## Args
Arguments stored in the Script structure in type or lock script values of a cell. Args are used as inputs to code stored in a cell referenced by its hash.

---

## First-Class Assets
A unique property of CKB wherein ownership of a Cell, and the data contained within, is not assigned by the issuer, developer, or smart contract. The user owns the cell and is responsible for costs associated with persistence on CKB.

#### See Also
- [Cell](#cell)
- [Cell Model](#cell-model)
- [First-Class Asset on the Nervos Network Blog](https://medium.com/nervosnetwork/first-class-asset-ff4feaf370c4)

---

## Capacity
The maximum amount of space (in bytes) that a Cell can occupy on the CKB blockchain.

#### Synonyms
- [CKBytes](#ckbytes)

#### See Also
- [CKB](#ckb)

---

## Cell
#### Synonyms
#### Not To Be Confused With
#### See Also

---

## Cell Model

#### See Also
- [Cell Model on the Nervos Blog](https://medium.com/nervosnetwork/https-medium-com-nervosnetwork-cell-model-7323fca57571)

---

## Cell Collection
A process of gathering cells that meet a certain criteria. For example- to find the balance of a particular account, all active cells for the address would need to be collected.

---

## Cellbase
Transaction in each block that creates new CKB. Analogous to Bitcoin's coinbase transaction. 

---

## CKB
An abbreviation which can have different meanings depending on the context:

- Common Knowledge Base
- Common Knowledge Byte

#### Synonyms
- [Common Knowledge Base](#common-knowledge-base-ckb)
- [Common Knowledge Byte](#common-knowledge-byte-ckb)

---

## CKByte
A shorthand name for Common Knowledge Byte.

#### Synonyms
- [CKB](#ckb)
- [Common Knowledge Byte](#common-knowledge-byte-ckb)

---

## CKB-VM

---

## Commitment Zone
Section of the block that contains transaction commitments. The commitment zone can only contain valid transactions which have appeared in the proposal zone of one of the previous 2 to 10 blocks.

---

## Common Knowledge Base (CKB)

---

## Common Knowledge Byte (CKB)

#### Synonyms
- [CKByte](#ckbyte)

---

## Consume

#### See Also
- [Cell](#cell)
- [Cell Model](#cell-model)
- [UTXO](https://bitcoin.org/en/glossary/unspent-transaction-output)

---

## Crypto Primitives

---

## Cycles
The number of RISC-V computational cycles required by a script to execute.

This is a similar concept to Ethereum's Gas.

#### See Also
- [Gas](https://github.com/ethereum/wiki/wiki/Glossary)
- [Script](#script)
- [RISC-V](#risc-v)

---

## Dead Cell
A cell that has been used as an input to a previous transaction. It cannot be used as an input to a new transaction. Analogous to a "spent UTXO" in Bitcoin.

#### See Also
- [Cell](#cell)
- [Cell Model](#cell-model)
- [UTXO](https://bitcoin.org/en/glossary/unspent-transaction-output)

---

## Deps
A shorthand name for dependencies.

#### Synonyms
- [Dependencies](#dependencies)

---

## Dep Group

---

## Dep Type

---

## Dependencies

---

## Eaglesong
The proof of work function for Nervos CKB. Spec can be found here 

#### See Also
- [Eaglesong RFC](https://github.com/nervosnetwork/rfcs/blob/master/rfcs/0010-eaglesong/0010-eaglesong.md)
---

## Epoch

---

## Generator

---

## Governance Script Hash (UDT)

---

## Historical Cell

---

## Input

---

## Keyper

---

## Layer 1
A proof of work blockchain known as the Common Knowledge Base (CKB) that serves as the storage persistence layer for the Nervos Network.

#### Synonyms
- [CKB](#ckb)
- [Common Knowledge Base](#common-knowledge-base-ckb)

---

## Layer 2

---

## Light Client

---

## Lina

---

## Live Cell
A Cell that has not been consumed and is available for use.

This is similar to an unspent transaction output (UTXO) in Bitcoin.

#### See Also
- [Cell](#cell)
- [Cell Model](#cell-model)
- [UTXO](https://bitcoin.org/en/glossary/unspent-transaction-output)

---

## Lock Script

---

## Long Address

---

## Molecule

---

## Muta Framework

---

## Nervos DAO

---

## Off-Chain State

---

## Off-Chain Computation

---

## On-Chain State

---

## Open Transaction

---

## Outpoint

---

## Output

---

## Proposal Zone

---

## RISC-V

---

## Script

---

## Secondary Issuance

---

## Shannon
A fractional denomination of CKBytes. One CKByte is equal to 100,000,000 Shannons.

A Shannon is the equivalent of a Bitcoin Satoshi.

#### See Also
- [CKByte](#ckbyte)
- [Common Knowledge Byte](#common-knowledge-byte-ckb)
- [Satoshi (denomination)](https://bitcoin.org/en/glossary/denominations)

---

## Since

---

## State Rent

---

## Store of Value

---

## SUDT

---

## Type ID

---

## Type Script

---

## UDT
An abbreviation of User-Defined Token.

#### Synonyms
- [User-Defined Token](#user-defined-token)

---

## User-Defined Token (UDT)
A unique non-fungible token with properties defined by the user.

A UDT is equivalent of an Ethereum ERC20 token or ER777 token.

#### See Also
- [ERC20](https://eips.ethereum.org/EIPS/eip-20)
- [ERC777](https://eips.ethereum.org/EIPS/eip-777)

---

## Validator

---

## Witness

---
